...
internal sealed class ExplorerHeader
{
	...
	internal ExplorerHeader(QueryExplorer explorer)
	{
		...
		entityStore.CreateEntity(new EntityName("Hello friflo EcGui"), default(Tags));
		entityStore.CreateEntity(new EntityName("--- add stores, queries or systems ---"), default(Tags));
		entityStore.CreateEntity(new EntityName("EcGui.AddExplorerStore()"), default(Tags));
		entityStore.CreateEntity(new EntityName("EcGui.AddExplorerQuery()"), default(Tags));
		entityStore.CreateEntity(new EntityName("EcGui.AddExplorerSystems()"), default(Tags));
	...
	internal void DrawExplorerMode()
	{
		...
		AlignRight alignRight = new AlignRight("...", right, fixedWidth: true, UI.Scl(41f));
		AlignRight alignRight2 = new AlignRight(" columns ", alignRight.left);
		AlignRight alignRight3 = new AlignRight("find", alignRight2.left, fixedWidth: false, ImGui.GetFrameHeightWithSpacing());
	...
	private void DrawSettings()
	{
		...
		if (kickIt)
		{
			...
			iO = ImGui.GetIO();
			ref float fontGlobalScale = ref iO.FontGlobalScale;
			if (num > 1f)
			{
				kickIt = false;
	...
	internal void DrawQuerySelector()
	{
		ImGuiStylePtr style = ImGui.GetStyle();
		float right = ImGui.GetWindowWidth() - UI.Scl(4f) - style.WindowPadding.X;
		AlignRight alignRight = new AlignRight("friflo EcGui", right);
		...
		expandQuery = ImGui.TreeNodeEx("query  ", ImGuiTreeNodeFlags.SpanTextWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere);
		ImGui.PopStyleColor();
		if (enableTooltips)
		{
		...
		if (num)
		{
			...
			expandQueries = ImGui.TreeNodeEx("Queries", ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere);
			string key;
			if (expandQueries)
			{
				int num2 = 0;
				...
				{
					query.Deconstruct(out key, out var _);
					string text = key;
			...
			if (systems.Count > 0)
			{
				...
				{
					system.Deconstruct(out key, out var value2);
		...
		ImGui.PopStyleColor();
		if (selectedQueryName != null && queries.TryGetValue(selectedQueryName, out var value3) && value3.saved)
		{
			ImGui.SameLine();
			ImGui.SetCursorPosY(ImGui.GetCursorPosY() - y);
	...
	private void DrawQueryFilter()
	{
		...
		ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags.SpanFullWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere;
		ImGui.PushStyleColor(ImGuiCol.Text, GlobalColors.queryText);
		...
		{
			if (DrawComponentFilter(1, "with all", condition.AllComponents, out var result))
			{
		...
		if (expandQueryTags)
		{
			if (DrawTagFilter(5, "with all", condition.AllTags, out var value))
			{
	...
	{
		collapsedSystems.TryGetValue(group, out var value);
		bool nextItemOpen = !value;
		ImGui.SetNextItemOpen(nextItemOpen);
		nextItemOpen = ImGui.TreeNodeEx(group.Name, ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere);
	...
	private string NewQueryName(string name)
	{
		...
		{
			if (int.TryParse(name.Substring(num), out var result))
			{
				num2 = result + 1;
