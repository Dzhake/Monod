...
internal sealed class ColumnSelector
{
	...
	private const ImGuiTreeNodeFlags TreeFlags = ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.NavLeftJumpsBackHere;

	private const ImGuiItemFlags ImGuiItemFlags_MixedValue = (ImGuiItemFlags)4096;

	private static MemberNode[] ComponentMembers => componentMembers ?? (componentMembers = CreateAllComponentMembers());
	...
	private static MemberNode[] CreateAllComponentMembers()
	{
		...
		foreach (ComponentType value in EntityStore.GetEntitySchema().ComponentTypeByType.Values)
		{
			...
			MemberPath path = MemberPath.Get(value.Type, "");
	...
	private void SortItems(SelectSortType sortType)
	{
		...
		{
			selection.TryGetValue(x.path, out var value);
			selection.TryGetValue(y.path, out var value2);
	...
	private void TraverseNode(MemberNode node)
	{
		...
		for (int i = 0; i < children.Length; i++)
		{
			...
			TraverseNode(node2);
			selection.TryGetValue(node2.path, out var value);
	...
	{
		selection.TryGetValue(node.path, out var value);
		if (triState != value)
		{
	...
	private void AddRemoveColumn(MemberNode node, TriState triState)
	{
		...
		{
			MemberDrawer memberDrawer = MemberDrawer.Create(node.path);
	...
	{
		selection.TryGetValue(member.path, out var value);
		if (member.children.Length == 0)
		{
			ImGui.PushItemFlag(ImGuiItemFlags.NoTabStop, enabled: true);
			bool num = ImGui.TreeNodeEx("##leaf", ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.Leaf | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.NavLeftJumpsBackHere);
			ImGui.PopItemFlag();
			if (num)
			{
				ImGui.SameLine();
		...
		expandMembers.TryGetValue(member.path, out var value2);
		ImGui.SetNextItemOpen(value2);
		ImGui.PushItemFlag(ImGuiItemFlags.NoTabStop, enabled: true);
		bool flag = ImGui.TreeNodeEx("##branch", ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.NavLeftJumpsBackHere);
		ImGui.PopItemFlag();
		if (value2 != flag)
		{
