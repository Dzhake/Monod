...
internal sealed class MultiRowComponent : IInspectorComponent
{
	...
	public void DrawComponentNode(DrawNode drawNode)
	{
		...
		ImGuiTreeNodeFlags imGuiTreeNodeFlags = ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere;
		...
		if (drawNode.ShowTooltips && ImGui.BeginItemTooltip())
		{
			...
			ImGui.Text(TextUtils.AsSpan());
			ImGui.EndTooltip();
		}
		ImGui.PopItemFlag();
		ImGui.PushStyleVar(ImGuiStyleVar.FrameRounding, 5f);
		InspectorCommand inspectorCommand = InspectorCommand.None;
		if (EntityInspector.MorePopup("generic_more", out var isFocused))
		{
