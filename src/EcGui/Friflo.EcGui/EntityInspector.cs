...
public sealed class EntityInspector
{
	...
	internal const ImGuiTreeNodeFlags TreeFull = ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.SpanAvailWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere;

	internal const ImGuiTreeNodeFlags TreeLabel = ImGuiTreeNodeFlags.AllowOverlap | ImGuiTreeNodeFlags.FramePadding | ImGuiTreeNodeFlags.SpanTextWidth | ImGuiTreeNodeFlags.NavLeftJumpsBackHere;
	...
	private void DrawComponentsSection()
	{
		...
		ImGui.Text(TextUtils.IntAsSpan(componentTypes.Count));
		ImGui.PopItemFlag();
		ImGui.PopStyleColor();
		if (AddButton(-2))
		{
	...
	private IInspectorComponent CreateInspectorComponent(ComponentType componentType)
	{
		...
		{
			MemberDrawer memberDrawer = MemberDrawer.Create(memberPath);
	...
	private static IMember[] CreateMembers(MemberPath basePath, int level)
	{
		...
		{
			MemberDrawer fieldDrawer = MemberDrawer.Create(memberPath);
	...
	private bool DrawHeader()
	{
		...
		AlignRight alignRight2 = new AlignRight("##filter", alignRight.left, fixedWidth: true, UI.Scl(200f));
		AlignRight alignRight3 = new AlignRight("filter", alignRight2.left);
		...
		int.TryParse(idText, out var result);
		...
		if (showAllocation && ImGui.GetCursorPosX() + UI.Scl(160f) < alignRight3.left)
		{
			...
			ImGui.Text(" heap");
			ImGui.SameLine();
			ImGui.Text(TextUtils.LongAsSpan(allocation));
		}
	...
	private static bool AddButton(int id)
	{
		...
		Vector2 size = new Vector2(UI.Scl(100f), ImGui.GetTextLineHeight());
	...
	internal static TRelation[] ClearRelations<TRelation, TKey>(Entity entity) where TRelation : struct, IRelation<TKey>
	{
		...
		TRelation[] array = new TRelation[relations.Length];
		int num = 0;
	...
	{
		InspectorRelation inspectorRelation = new InspectorRelation(MemberDrawer.Create(MemberPath.Get(typeof(Relations<>).MakeGenericType(relationType.Type), "")), relationType);
	...
	private void DrawRelationsSection()
	{
		...
		ImGui.SameLine();
		ImGui.Text(TextUtils.IntAsSpan(relations.Count));
		ImGui.PopItemFlag();
		ImGui.PopStyleColor();
		if (AddButton(-3))
		{
	...
	private void DrawTagsSection()
	{
		...
		ImGui.SameLine();
		ImGui.Text(TextUtils.IntAsSpan(entity.Tags.Count));
		ImGui.PopItemFlag();
		ImGui.PopStyleColor();
		if (AddButton(-1))
		{
	...
	private void DrawTags()
	{
		...
		Vector2 size = new Vector2(x, ImGui.GetTextLineHeightWithSpacing());
		int num3 = 0;
